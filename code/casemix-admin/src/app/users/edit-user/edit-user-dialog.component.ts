...
export class EditUserDialogComponent extends AppComponentBase implements OnInit {
  ...
  currentUserHasSuper: boolean = false;
  ...
  @ViewChild('assignHospitalsComponent') assignHospitalsComponent: AssignHospitalsComponent;
  @ViewChild('assignSpecialtiesComponent') assignSpecialtiesComponent: AssignSpecialtiesComponent;
  ...
  ngOnInit(): void {
    ...
    this.getRegionsMultiselected(this.id);
    this.getHospitalsMultiSelected(this.id);

    this._userService.get(this.id).subscribe((result) => {
      this.user = result;

      this.selectedManfacturer = this.user.manufactureId;
      ...
      this._userService.getRoles().subscribe((result2) => {
        ...
        if (!this.currentUserHasSuper) {

          if (this.checkedRolesMap['SUPER ADMIN'] === true) {
  ...
  getAllRegions(): void {
    this._regionsService.getAllRegionData(true, this.id).subscribe((result: RegionManagementNodeDto[]) => {
      this.regions = result;
      _.forEach(result, (res) => {
        _.forEach(res.children, (child) => {
          if (child.selected === true) this.selectedNodes.push(child);
        });
      });
  ...
  setInitialRolesStatus(): void {
    ...
    if (this.checkedRolesMap['MANUFACTURER'] === true) {
        this.otherRolesRadio.forEach((element) => {
          element.nativeElement.checked = true;
        });

        this.superAdminRadio.forEach((element) => {
    ...
    } else if (this.checkedRolesMap['SUPER ADMIN'] === true) {
        this.otherRolesRadio.forEach((element) => {
          element.nativeElement.checked = false;
        });
  ...
  save(): void {
    ...
    this._userService
      ...
      .subscribe(() => {
        _.forEach(this.userHospitals, (userHospital) => {
        ...
        this.regionHospitalMapping.userId = this.user.id;

        this._regionsService.saveUserRealm(this.regionHospitalMapping).subscribe(() => {});

        if (this.assignSpecialtiesComponent !== undefined) {
          this.assignSpecialtiesComponent.saveSurgeonSpecialties(this.user.id).subscribe(() => {});
        }
  ...
  getRegionsMultiselected(id: any): void {
    this._regionsService.getAllRegionsMultiSelect(id).subscribe((result) => {
      _.forEach(result, (res) => {
        this.selectedNodes.push(res);
      });
    });
  }

  getHospitalsMultiSelected(id: any): void {
    this._hospitalsService.getAllHospitalsMultiSelect(id).subscribe((result) => {
      _.forEach(result, (res) => {
        this.selectedNodes.push(res);
      });
  ...
  private getUserHospitals(): void {
    this._userHospitalsService.getAll(this.user.id).subscribe((userHospitals) => {
  ...
  private getCurrentUserDetails(): void {
    this._userService.get(this.appSession.userId).subscribe((userInfo) => {
      this.currentUserHasSuper = userInfo.roleNames.find((u) => u === 'SUPER ADMIN') === 'SUPER ADMIN';      
    });
  }
}
