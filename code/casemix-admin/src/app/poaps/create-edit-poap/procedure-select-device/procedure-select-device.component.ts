...
@Component({
  selector: 'app-procedure-select-device',
  templateUrl: './procedure-select-device.component.html',
  styleUrls: ['./procedure-select-device.component.less'],
  animations: [appModuleAnimation()],
  providers: [ManufacturerDevicesModel]
})

export class ProcedureSelectDeviceComponent extends AppComponentBase implements OnInit {
  ...
  devices: DeviceDto[] = [];
  modelSearchDevices: DeviceDto[] = [];

  selectedDevices: DeviceDto[] = [];
  isLoading = false;

  manufacturerDevices: ManufacturerDevicesModel[] = [];
      ...
      .subscribe((res) => {
          if (res){
             this.devices = res;
              ...
              this.modelSearchDevices = this.devices;
          }
      });
  }

  prepareManufacturerDevices(devices_: DeviceBrandFamilyDto[]): ManufacturerDevicesModel[] {
    const ret: ManufacturerDevicesModel[] = [];
    var newSet = _.groupBy(devices_, d => d.manufacturerId);

    _.forEach(newSet, (n) => {
      let manufactureDevice = new ManufacturerDevicesModel;
      manufactureDevice.manufacturerId = n[0].manufacturerId;
      manufactureDevice.manufacturerName = n[0].manufacturerName;
      manufactureDevice.deviceBrandFamily = n;

      ret.push(manufactureDevice);
    });
